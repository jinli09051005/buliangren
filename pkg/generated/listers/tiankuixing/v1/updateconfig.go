...
type UpdateConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1.UpdateConfig, err error)
...
// List lists all UpdateConfigs in the indexer.
func (s *updateConfigLister) List(selector labels.Selector) (ret []*v1.UpdateConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.UpdateConfig))
	})
	return ret, err
}
...
type UpdateConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.UpdateConfig, error)
...
// List lists all UpdateConfigs in the indexer for a given namespace.
func (s updateConfigNamespaceLister) List(selector labels.Selector) (ret []*v1.UpdateConfig, err error) {
...
// Get retrieves the UpdateConfig from the indexer for a given namespace and name.
func (s updateConfigNamespaceLister) Get(name string) (*v1.UpdateConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("updateconfig"), name)
	}
	return obj.(*v1.UpdateConfig), nil
}
